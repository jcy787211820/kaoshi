...
class Manager
{
	...
	public function insert($option = array())
	{
		 ...
		 */
		$need_trans_action	= Database::getTransActionStatus() == FALSE && $this->create_log == TRUE;
		...
		/*
		 * check create log data is normal
		 */
		if( $this->create_log == TRUE && count( $field_values ) > 1 ) throw new CException('If insert log, then insert affected rows must be one row.');
		...
		try
		{
			...
			if($this->create_log == TRUE )
			{
				$log_manager					= Go::manager('Log');
				$log_manager->log_type			= $log_manager::LOG_TYPE_INSERT;
				$log_manager->log_table			= $this->table;
				$log_manager->log_key			= $this->{$this->primary_key};
				$log_manager->log_data			= $this->fields;
				$log_manager->insert();
			}
	...
	{
		if( $this->create_log == TRUE && empty( $this->log_keys ) == TRUE ) throw new CException('If insert log, then update primary key must be set.');
		...
		try
		{
			...
			{
				$log_manager					= Go::manager('Log');
				$log_manager->log_type			= $log_manager::LOG_TYPE_UPDATE;
				$log_manager->log_table			= $this->table;
				$log_manager->log_data			= $this->fields;
				$log_manager->insertByLogKeys( $this->log_keys );
			}
	...
	public function delete()
	{
		...
		try
		{
			...
			{
				$log_manager					= Go::manager('Log');
				$log_manager->log_type			= $log_manager::LOG_TYPE_DELETE;
				$log_manager->log_table			= $this->table;
				$log_manager->log_data			= array();
				$log_manager->insertByLogKeys( $this->log_keys );
			}
