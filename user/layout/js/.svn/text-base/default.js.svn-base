...
var pageList	= function( option ){
	...
	var pageBar	= function( total ){
		...
		if( pag_bas > 0 ){
			...
			tag.push('第<input type="text" name="goPage" value="' + pag_now + '" size="3" class="text-center" />页');
			tag.push('<input type="button" value="跳转" class="js_page_list_i btn btn-default btn-sm" />');
		}
		jQuery(page_tag).html(tag.join("\n"));
		
		jQuery('.js_page_list_i.on').css('background','Yellow');
		
		jQuery('.js_page_list_i:not(.on)').click(function(){
