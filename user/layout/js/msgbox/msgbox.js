...
var msgbox	= function( option ){
	...
	if(typeof( option.buttons ) == 'undefined'){
		switch( type ){			
			case 'confirm':
				buttons	= [{'type' : 'yes', 'value': '是'}, {'type' : 'no', 'value': '否'}];
				break;
			case 'prompt':
				buttons	= [{'type' : 'yes', 'value': '确定'}];
				break;
			case 'alert':
			case 'info':
			case 'error':
			default:
				buttons	= [{'type' : 'close', 'value': '确定'}];
		}
	}else{
	...
	for(var i in buttons){
		...
		var button_value	= typeof( buttons[i].value ) == 'undefined' ? '确定' : buttons[i].value;
		button_tags[i]	= 
